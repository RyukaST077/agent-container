...
**Tests are OPTIONAL (with one exception)**: Unit tests and contract tests are only generated if explicitly requested in the feature specification or if the user requests a TDD approach.

**EXCEPTION — Final verification via Playwright (ALWAYS generated)**: Regardless of whether other tests are requested, you MUST always generate a final verification phase that confirms the implemented feature actually works through the UI. For web/frontend features, this MUST include Playwright-based end-to-end screen tests that drive the real browser and assert on the rendered screen for each user story's primary acceptance scenario. Use the project's existing Playwright setup (e.g. `playwright.config.ts`, `e2e/`) when present. If the project has no UI (pure CLI/library), substitute an equivalent end-to-end execution check and note why Playwright is not applicable.
