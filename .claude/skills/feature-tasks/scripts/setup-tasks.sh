...
if [[ ! -f "$FEATURE_SPEC" ]]; then
    ...
    echo "Run /feature-specify first to create the feature structure." >&2
    exit 1
fi
...
if [[ -z "$TASKS_TEMPLATE" ]] || [[ ! -f "$TASKS_TEMPLATE" ]]; then
    echo "Warning: tasks template not found for $REPO_ROOT; caller should fall back to the skill-bundled reference/tasks-template.md" >&2
    TASKS_TEMPLATE=""
fi
