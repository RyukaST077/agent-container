# consult-on-prompt.ps1  —  UserPromptSubmit hook (PowerShell port of consult-on-prompt.sh)
#
...
#
#   Pure PowerShell — no external dependencies (no jq / grep). Works on Windows
#   PowerShell 5.1 and PowerShell 7+. Always exits 0 (non-blocking).
#
...
try {
    ...
    if ($prompt -imatch $pattern) {
        ...
        $out = [ordered]@{ hookSpecificOutput = [ordered]@{ hookEventName = 'UserPromptSubmit'; additionalContext = $msg } }
