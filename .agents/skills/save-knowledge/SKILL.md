...
2. Ask the user **once, in a single batch**, only for fields that can't be filled or that you're
   ...
   - Ask in one concise batch; use request_user_input only when available and appropriate.
...
1. Generate the save path deterministically (handles `knowledge/` creation, date prefix, collisions):
   ...
   ```
   # macOS / Linux (bash)
   bash .agents/skills/save-knowledge/scripts/new-knowledge.sh "<slug>"

   # Windows (PowerShell)
   powershell -NoProfile -ExecutionPolicy Bypass -File .agents/skills/save-knowledge/scripts/new-knowledge.ps1 "<slug>"
   ```
...
- **Don't write before approval**: no file write / INDEX update until Phase 3 passes.
