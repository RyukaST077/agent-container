...
スキルは本来「Claudeが自発的に気づいたら発火」ですが、それを取りこぼさないよう
`.claude/hooks/` の3つのフックが発火を後押しします（`.claude/settings.json` に登録）。
...
- うるさい / 不要なら `.claude/settings.json` の `hooks` から該当エントリを外せば無効化できます。
